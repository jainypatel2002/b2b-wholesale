...
const acceptedStatuses = new Set(['accepted', 'approved', 'confirmed'])

/**
 * Mechanism note:
 * - DB trigger on orders status transition to accepted enqueues ORDER_ACCEPTED in email_events.
 * - This webhook-compatible function performs the same enqueue RPC (idempotent) and
 *   nudges process-email-events for near-real-time delivery.
 */
...
serve(async (req) => {
  try {
    ...
    if (!orderId || !record) {
      return new Response(JSON.stringify({ ok: true, action: 'ignored', reason: 'missing order payload' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
    }
    ...
    if (!acceptedStatuses.has(newStatus) || acceptedStatuses.has(oldStatus)) {
      return new Response(JSON.stringify({
        ok: true,
        action: 'ignored',
        reason: `status transition ${oldStatus || 'null'} -> ${newStatus || 'null'} is not a fresh acceptance`,
        order_id: orderId
      }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
    ...
    const { error: enqueueError } = await supabase.rpc('enqueue_order_accepted_email_for_order', {
      p_order_id: orderId
    })

    if (enqueueError) {
      throw new Error(`enqueue_order_accepted_email_for_order failed: ${enqueueError.message}`)
    }
    ...
    try {
      ...
      console.error(`[notify-order-accepted] worker invoke failed for order=${orderId}`, workerError)
    }
