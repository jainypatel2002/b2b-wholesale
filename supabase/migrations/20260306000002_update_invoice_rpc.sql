-- Migration: Update generate_invoice RPC for Canonical Model
-- Ensures snapshots include unit_price_snapshot and case_price_snapshot
...
begin
  ...
  -- 5. Snapshot Items (Fixed for Canonical Model)
  insert into public.invoice_items (
    invoice_id, product_id, product_name, qty, unit_price, unit_cost,
    unit_price_snapshot, case_price_snapshot, -- Canonical Snapshots
  ...
  select 
    ...
    unit_cost,
    -- Snapshots: Use edited price as the NEW snapshot for the invoice, 
    -- while preserving original snapshot if not edited.
    coalesce(edited_unit_price, unit_price),
    case 
        when order_unit = 'case' then coalesce(edited_unit_price, unit_price)
        else coalesce(case_price_snapshot, unit_price * units_per_case_snapshot)
    end,
    order_unit, cases_qty, pieces_qty, units_per_case_snapshot, total_pieces,
    coalesce(edited_qty, qty),
